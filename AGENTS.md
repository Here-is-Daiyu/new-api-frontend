...
9. 每次完成修改后，先查看 `view_diff` 再提供建议 commit message（仅提供文案，不执行 git commit/push）。

## 本次要实现的功能范围 / 任务项（2026-02-24）

1. 修复“拉取模型列表”在服务要求令牌时出现“未提供令牌”的问题。
2. 模型面板在存在可用 Token 时默认选中首个 API Key，且保留手动切换能力。
3. 优化模型与日志筛选下拉框 UI，解决长文本导致的内容拥挤/错位显示问题。
4. `/favicon.png` 改为通过 VPS 本地代理透传（不再直连远端域名）。
5. 日志审计 Tab 在未打开前后台预拉取第一页，提升首次打开体验。
6. 日志审计 Tab 在“全部 Token”场景下由 VPS 端聚合请求各 Token 模型并去重返回。

## 本次任务完成情况（2026-02-24）

- [x] 修复模型列表在需令牌场景下“未提供令牌”问题（默认选中可用 Key 并保留手动切换）。
- [x] 优化模型与日志筛选下拉框长文本显示与可读性。
- [x] `/favicon.png` 改为本地 VPS 透传。
- [x] 日志审计 Tab 未打开前后台预拉取第一页。
- [x] 日志审计“全部 Token”模型下拉改为 VPS 端聚合去重返回。
