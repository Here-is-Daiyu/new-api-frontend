...
## 本次要实现的功能范围 / 任务项（2026-02-24 第三轮）

1. 修复 Token 面板筛选区页大小下拉（`tokenPageSizeSelect`）异常渲染问题（箭头重复/重影）。
2. 将日志审计开始/结束时间筛选从 `datetime-local` 改为普通输入框（文本输入）。
3. 将日志审计“模型”筛选从原生 `datalist` 改为前端自绘下拉，避免大面板遮挡视线。
4. 日志审计“分组”尝试从 BaseURL 关联数据动态获取并提供下拉；若无法获取可选分组则展示提示文案。

## 本次任务完成情况（2026-02-24 第三轮）

- [x] 修复 Token 面板下拉异常渲染（页大小下拉改为前端自绘，消除原生箭头重复/重影）。
- [x] 日志时间筛选改为文本输入（支持常见日期时间文本与时间戳解析）。
- [x] 日志模型筛选改为自绘下拉并保留动态刷新（按 Token 维度刷新模型候选）。
- [x] 日志分组支持动态下拉与无数据提示（优先尝试用户可用分组接口）。
