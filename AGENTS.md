...
## 本次要实现的功能范围 / 任务项（2026-02-27 第四轮）

1. 日志审计表格“耗时”列改为“原耗时 - 首字耗时”后再展示。
2. 日志审计表格新增“输出速率”列，按 `Completion / 更改后耗时` 计算。
3. 日志审计表头文案调整：`Prompt` 改为“输入”，`Completion` 改为“输出”。

## 本次任务完成情况（2026-02-27 第四轮）

- [x] 日志审计表格“耗时”列改为“原耗时 - 首字耗时”后再展示。
- [x] 日志审计表格新增“输出速率”列，按 `Completion / 更改后耗时` 计算。
- [x] 日志审计表头文案调整：`Prompt` 改为“输入”，`Completion` 改为“输出”。
