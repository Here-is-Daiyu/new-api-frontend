...
## 本次要实现的功能范围 / 任务项（2026-02-24 第二轮）

1. 将日志审计筛选区与模型面板筛选区的关键下拉框改为前端自绘下拉（不使用原生 `select` UI 渲染），修复当前错位/重影问题。
2. 保留现有筛选逻辑与动态选项刷新能力（模型 API Key、日志 Token、日志类型）。
3. 日志审计表格滚动时固定表头，确保“表头不跟随数据区滚动”。

## 本次任务完成情况（2026-02-24 第二轮）

- [x] 自绘下拉替换完成并修复错位问题（模型 API Key、日志类型、日志 Token 均改为前端自绘）。
- [x] 筛选交互（默认值、手动切换、动态刷新）保持可用。
- [x] 日志审计表头固定完成（数据区滚动时表头保持可见）。
