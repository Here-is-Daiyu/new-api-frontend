...
## 本次要实现的功能范围 / 任务项（2026-03-27 第十二轮）

1. 统一 `light / dark theme` 下的页面背景层级，明确 `body / 主面板 / 次级表面` 三层关系，避免顶部导航继续使用与主题变量脱节的硬编码底色。
2. 重点修复 `Token` / `可用模型` / `日志审计` 页面，以及登录卡片、筛选区、统计卡片、加载提示、移动端 Token / 日志卡片内部小块的底色混用问题。
3. 保持桌面端现有交互、移动端紧凑布局，以及 Token Key 脱敏显示 / 完整复制逻辑不受影响。
4. 同步更新 `README.md` 与 `AGENTS.md`，明确新的主题统一策略与本地编译后再浏览器复查的流程说明。

## 本次任务完成情况（2026-03-27 第十二轮）

- [x] 已完成主要页面 / 模块底色层级统一修复。
- [x] 已覆盖 light / dark theme 与桌面 / 移动端一致性检查。
- [x] 已同步更新 README / AGENTS 说明。
- [x] 已完成本地基础验证（至少 `go build`，必要时补充前端静态校验）。
