...
- 项目绝大部分代码由 AI 编写，人工主要负责需求确认、验收、部署与回归把关。
...
## 本次要实现的功能范围 / 任务项（2026-03-28 第十六轮）

1. 修复真实手机浏览器下整个页面疑似无法上下滚动的问题，优先排查 `html/body/app-shell` 高度与滚动容器设置是否错误。
2. 修复真实手机浏览器下页面呈现“像桌面版被固定宽度缩放/居中裁切”的回归问题，确保移动端断点样式真正生效。
3. 保持第十五轮已完成的 `/console/log` 按钮、Token Key 默认隐藏、SVG 图标替换与移动端紧凑布局目标不被破坏。
4. 本轮继续采用“本地修改 -> 本地构建 -> VPS 覆盖 -> 真实浏览器强制刷新复查”流程验收。
5. 若问题与 viewport / 缓存 / 移动端宽度策略有关，需要在修复后再次确认 Token 面板不再出现桌面表格横向裁切。
6. 本次代码修复范围限定为 `web/index.html`、`web/style.css` 及文档同步，优先做 viewport 与根布局最小修复，不重做既有交互逻辑。

## 本次任务完成情况（2026-03-28 第十六轮）

- [x] 已完成移动端滚动问题定位与修复。
- [x] 已完成移动端固定宽度 / 桌面缩放样式回归修复。
- [ ] 已验证 Token / Log 页面在真实手机浏览器下均可正常纵向浏览。
- [ ] 已完成本地验证、VPS 部署与真实浏览器复查。
- [x] 已同步更新 README / AGENTS 说明（如修复影响用户说明）。

补充说明：当前已根据真实手机截图确认存在新回归——页面表现接近桌面固定宽度且难以上下滑动；本轮先修复该回归，再重新执行完整浏览器验收。

本地已完成 `go build ./...`、`node --check web/app.js` 与 `git diff --check`，且已完成 VPS 覆盖部署；Chrome 移动视口下已确认页面可纵向滚动且无横向滚动，真实手机强制刷新复查仍待最终确认。
...
2. 涉及 UI 或部署相关改动时，默认按“本地编译 -> 推到 VPS 覆盖 -> 真实浏览器复查通过后再汇报 / 再决定是否提交推送”的顺序验收；必要时可先补充本地自测。
3. VPS + 浏览器复查通过后，再决定是否提交并推送远端仓库。
...
8. 默认流程：`本地编译 -> 推到 VPS 覆盖 -> 真实浏览器复查通过后再汇报 / 再决定是否提交推送`。
9. 支持使用产物直链在 VPS 远端直接下载并部署（无需本地中转上传）。
10. 静态资源缓存策略默认采用“首页 HTML 禁缓存 + `style.css` / `app.js` 内容哈希版本参数”；部署后若页面仍像旧版，先检查首页引用的哈希参数是否已更新。
...
5. Token Key 默认完全隐藏，支持前端显示/隐藏完整值切换，复制时仍使用完整 Key。
6. 移动端布局采用 `768px` 与 `480px` 双断点响应式优化；其中 Token / 日志列表在手机端优先切换为卡片 / 栈式布局，避免依赖横向滚动。
7. 主题底色统一按 `body / 主面板 / 次级表面` 三层变量管理；顶部导航、筛选区、统计卡片、加载提示与移动端卡片内部小块禁止再写脱离主题变量的硬编码底色。
