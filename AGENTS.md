...
## 本次要实现的功能范围 / 任务项（2026-03-08 第七轮）

1. 所有刷新按钮统一为“保留原文案/图标 + 按钮边上显示加载 spinner”的交互。
2. Token / 模型 / 日志点击刷新时改为静默更新，保留当前列表内容，收到响应后再替换，避免打断浏览。
3. 复用通用 loading 逻辑，避免重复点击造成并发刷新。

## 本次任务完成情况（2026-03-08 第七轮）

- [x] 所有刷新按钮统一为“保留原文案/图标 + 按钮边上显示加载 spinner”的交互。
- [x] Token / 模型 / 日志点击刷新时改为静默更新，保留当前列表内容，收到响应后再替换，避免打断浏览。
