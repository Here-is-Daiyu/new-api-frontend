...
- ✅ 所有刷新按钮统一保留原文案/图标，加载时仅在按钮旁显示 spinner
- ✅ Token / 模型 / 日志点击刷新采用静默更新，保留当前列表内容，待响应返回后再替换
- ✅ 默认暗色模式 + 手动切换亮暗主题
...
- 顶部会话、Token、模型、日志的“刷新”按钮点击后会保留原有文案/图标，仅在按钮旁显示 loading spinner，并自动禁用按钮直到请求完成。
- Token / 模型 / 日志点击刷新时会保留当前列表与滚动位置，待新响应返回后再整体替换，避免打断浏览。
